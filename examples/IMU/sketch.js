...
function draw() {
    ...
    ambientLight(120);
    directionalLight(190, 190, 185, -0.4, -0.9, -0.5);  // soft top-left key
    pointLight(240, 225, 190, 220, -180, 280);            // warm front-right fill
    ...
    // Render the loaded model — house orange
    translate(0, -65, 0);
...
function drawPlane() {
    ...
    // PCB substrate — house teal slab
    fill('#5864e7');
    box(310, 12, 248);

    // Orientation marker — amber sphere at the +X edge of the board.
...
// House palette: orange = X, teal = Y, amber = Z (matches the HUD labels)
...
function setStatus(connected) {
    const el = id('status');
    el.textContent = connected ? 'status: ready' : 'status: waiting for connection…';
