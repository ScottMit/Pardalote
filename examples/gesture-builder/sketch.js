...
// remembered separately by connect.js under its own key.
...
// -------------------------------------------------------------------
// Bus RX/TX pin lock — a UNO R4's bus is fixed to Serial1 (D0/D1).
// (The connection standard itself lives in connect.js.)
// -------------------------------------------------------------------
...
const conn = setupConnection(arduino, { store: 'pardalote-gesture-builder-conn' });   // Board controls + connection
// Move the bus RX/TX controls onto the connection row (same row as WiFi/USB),
// then drop the now-empty Bus row. Uses the row connect.js hands back.
(function moveBusOntoConnRow() {
    const oldRow = rxEl.closest('.row');
    if (!oldRow || !conn.row) return;
    const busLbl = document.createElement('span');
    busLbl.textContent = 'bus'; busLbl.style.marginLeft = '8px'; busLbl.style.fontWeight = '700';
    conn.row.appendChild(busLbl);
    [...oldRow.children].forEach(ch => { if (!ch.classList.contains('lbl')) conn.row.appendChild(ch); });
    oldRow.remove();
})();
