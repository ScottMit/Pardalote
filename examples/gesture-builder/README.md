...
- **Board limits:** up to **12 segments per servo** and (here) **10 keyframes
  ...
  together padding.
- **Bus servos run their own motion.** Unlike a PWM `Servo` (where the board
  renders the easing *inside* each segment), a Feetech bus servo executes each
  segment as a single speed-matched move — so the easing **curve** you pick is
  faithfully drawn in the preview and encoded on the wire, but a bus servo won't
  visibly render an ease *within* one segment. To make a shape read on real
  bus-servo hardware, **decompose** the move into more, shorter segments (more
  keyframes) — the tool's per-segment timing then does the shaping. This is the
  authoring front-end; deeper on-hardware curve rendering for bus servos is a
  natural next step.
