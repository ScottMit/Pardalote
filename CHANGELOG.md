...
- **USB serial transport** — `Pardalote.begin(PARDALOTE_SERIAL)` +
  `arduino.connectSerial()` (Web Serial; Chrome/Edge) carry the same
  binary protocol over the USB cable: no network, no IP, one-line
  migration to WiFi later. COBS-framed with a CRC8, so the sketch's
  `Serial.print` output coexists on the wire and reaches the page as
  the `'log'` event. Adds **UNO R4 Minima** support (serial-only).
  Camera excepted (its video is an HTTP stream).
- **Connection keys (opt-in)** — `Pardalote.begin("key")` +
  `arduino.connect(ip, { key })`. Wrong/missing key → one `'authFail'`
  event and auto-reconnect stops. An accident-prevention latch for
  shared networks, not security (cleartext). WebSocket transport only;
  serial implies possession. Wire: `CMD_AUTH 0x0C`, folded into
  protocol v1.0 pre-release.
