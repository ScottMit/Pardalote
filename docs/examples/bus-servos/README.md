...
**File → Examples → Pardalote → bus-servos**. Note the IP the board reports.
...
3. Set the two joints' bus IDs in the **Servos** row under the display
   (defaults 1 and 2) — changing an ID reconnects and re-attaches.

The browser remembers the IP and IDs, so next visit it reconnects with one
click.

| Input | Action |
|---|---|
| Drag a dial | command that joint's position |
| Torque buttons or `1` / `2` | free that joint (torque off) to pose by hand; press again to re-hold |
