...
const STORE = 'pardalote-stepper-motor-cfg';
const DEFAULTS = {
...
function setup() {
    ...
    setupConnection(arduino, { store: 'pardalote-stepper-motor' });   // Board controls + connection

    // Wiring — driver mode + coil/step pins, grouped with the settings
    let r = row(main, 'Wiring');
    ...
    // Attach + configure on every (re)connect; clear the ready flag on a drop.
...
function refreshToggles() {
    ...
    // "disable (free)" goes green while the coils are free (the free/limp convention)
    if (!enabled) disableBtn.addClass('freed'); else disableBtn.removeClass('freed');
}
