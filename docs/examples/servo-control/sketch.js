...
const STORE = 'pardalote-servo-control-cfg';
const DEFAULTS = {
...
function setup() {
    ...
    const conn = setupConnection(arduino, { store: 'pardalote-servo-control' });   // Board controls + connection

    // Wiring — the servo pin (changing it reconnects on the new pin)
    ...
    pinIn.changed(() => { persist(); if (arduino.connected) conn.connect(); });
    ...
    // Configure the servo on every (re)connect; clear the ready flag on a drop.
