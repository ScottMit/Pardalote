...
Set the **trig** and **echo** pins at the top of `sketch.js` (`const TRIG` / `const ECHO`, defaults 12 and 14).
...
1. In Arduino IDE: **File → Examples → Pardalote → ultrasonic-sensor**. The sketch is two lines:
   ```cpp
   #include <Pardalote.h>
...
browser. The trig/echo pins are set at the top of `sketch.js` (`const TRIG` /
`const ECHO`). Move an object in front of the sensor — the bar should respond.

## How It Works

```javascript
const TRIG = 12, ECHO = 14;   // HC-SR04 trigger / echo pins

arduino = new Arduino();
arduino.add('ultrasonicSensor', new Ultrasonic());   // register the extension
setupConnection(arduino, { store: 'pardalote-ultrasonic' });   // Board row + connection (connect.js)

// Attach + start polling once the board is ready ('ready' re-fires on every reconnect)
arduino.on('ready', () => {
    arduino.ultrasonicSensor.attach(TRIG, ECHO);
